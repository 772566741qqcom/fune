...
public class B_MemberController extends B_BaseController {
    ...
    @ApiOperation(value = "关注客户列表")
    @RequestMapping(value = "/focus/list", method = RequestMethod.GET)
    public R<Page<MemberDO>> focusList(PageModel page) {
        MemberFocusQuery query = new MemberFocusQuery();
        query.setShopId(loginKeeperHolder.getLoginShopId());
        Page<MemberFocusDO> pager = (Page<MemberFocusDO>) memberFocusService.page(page, QueryUtil.buildWrapper(query));
        List<Long> memberIds = new ArrayList<>();
        pager.getRecords().forEach(item -> memberIds.add(item.getMemberId()));
        pager.setRecords(null);
        Page<MemberDO> result = new Page<>();
