...
public class B_ProductController extends B_BaseController {
    ...
    public R<Page<ProductDO>> page(PageModel page, ProductQuery query) {
        ...
        QueryWrapper<ProductDO> queryWrapper = QueryUtil.buildWrapper(query);
        queryWrapper.and(i -> i.eq("buyout", 1).or().eq("allot", 1));
        Page<ProductDO> pager = (Page<ProductDO>) productService.page(page, queryWrapper);
        return Response.restResult(pager, ResultCode.SUCCESS);
    }
    ...
    public R<ProductDO> select(@PathVariable Long id) {
        ...
        if (classify2 != null) {
            ProductClassifyDO classify1 = productClassifyService.getById(classify2.getParentId());
            if (classify1 != null) {
    ...
    public R<ProductDO> update(@RequestBody ProductDO product) {
        ...
        if (product.getSourceId() != null && product.getCrowd() != null && product.getCrowd() == 1) {
