...
import com.d2c.shop.c_api.support.OrderRequestBeanC;
...
public class C_OrderController extends C_BaseController {
    ...
    public R<OrderDO> settle(@RequestBody OrderRequestBeanC orderRequest) {
        // 参数校验
    ...
    public R<OrderDO> create(@RequestBody OrderRequestBeanC orderRequest) {
        // 参数校验
        List<Long> cartIds = orderRequest.getCartIds();
        Long skuId = orderRequest.getSkuId();
        Integer quantity = orderRequest.getQuantity();
    ...
    private String wxPay(OrderDO order, PaymentDO payment, String tradeType) throws Exception {
        WXPay wxPay = new WXPay(wxJsPayConfig, WxJsPayConfig.NOTIFY_URL);
    ...
    private String aliPay(OrderDO order, PaymentDO payment) throws Exception {
        ...
        payment.setAppId(AliPayConfig.APP_ID1);
        payment.setMchId("0");
